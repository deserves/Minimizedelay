...
class MemoryDNN:
    ...
    def knm(self, m, k=1):
        # return k+1 order-preserving binary actions
        m_list = []
        ...
        if k > 1:
            ...
            for i in range(k - 1):
                if m[idx_list[i]] > 0:
                    ...
                    m_list.append(1 * (m - m[idx_list[i]] >= 0))
        m_list1=self.opn(m, k)
        return m_list+m_list1
    # def knm(self, m, k=1):
    #     # return k+1 order-preserving binary actions
    #     m_list = []
    #     # generate the ﬁrst binary ofﬂoading decision with respect to equation (8)
    ...
    def opn(self, m, k=1):
        ...
        m_list = []
        # generate the ﬁrst binary ofﬂoading decision with respect to equation (8)
        m_noise = m + np.random.normal(0, 1, len(m))
        m_list.append(1*(m_noise > 0))
        ...
        if k > 1:
            ...
            idx_list = np.argsort(m_abs)[:k - 1]
            for i in range(k - 1):
                if m_noise[idx_list[i]] > 0:
                    # set a positive user to 0
                    m_list.append(1 * (m_noise - m_noise[idx_list[i]] > 0))
                else:
                    # set a negtive user to 1
                    m_list.append(1 * (m_noise - m_noise[idx_list[i]] >= 0))
        return m_list
